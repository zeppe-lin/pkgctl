...
require_line "$srcdir/include/pkgctl/version.h" \
  'inline constexpr const char* version_string = "0.40.3";'
...
require_dependency_range libpkgapply-posix '>=3.2.2' '<4.0.0'
...
trap 'rm -f "$temporary.current" "$temporary.deps" "$temporary.027" "$temporary.unreleased" "$temporary.0403" "$temporary.0402" "$temporary.0401" "$temporary.040" "$temporary.039"' EXIT HUP INT TERM
