...
body=$(sed -n '/^transaction_run_admission_checkpoint admit_transaction_run(/,/^} \/\/ namespace$/p' \
  "$source")
previous=0
for token in \
  'transaction_run::begin' \
  ...
  [ -n "$line" ] && [ "$line" -gt "$previous" ] || {
    echo "invalid durable run-admission order: $token" >&2
    exit 1
  }
  previous=$line
done
