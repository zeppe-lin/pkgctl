...
constexpr std::string_view marker_payload = "hostile-authority\n";
...
public:
  ...
  void begin(const pkgimage::package_entry& entry) override
  {
    if (began_ || ended_ || entry.id != expected_)
      throw std::runtime_error("unexpected hostile marker replay begin");
    began_ = true;
  }
  ...
  {
    if (!began_ || ended_ || entry.id != expected_)
      throw std::runtime_error("unexpected hostile marker replay write");
  ...
  {
    if (!began_ || ended_ || entry.id != expected_)
      throw std::runtime_error("unexpected hostile marker replay end");
    ended_ = true;
  }
...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    if (marker == nullptr)
      throw std::runtime_error("sealed hostile image lacks shared marker");
    if (marker->type != pkgimage::entry_type::regular)
      throw std::runtime_error("hostile shared marker is not a regular file");
    if (marker->mode != 0644U)
      throw std::runtime_error("hostile shared marker mode differs from 0644");
    if (marker->size != marker_payload.size())
      throw std::runtime_error("hostile shared marker size differs from authority");
    if (!marker->regular_content ||
        marker->regular_content->string() != marker_digest)
      throw std::runtime_error("hostile shared marker content identity differs");
    ...
    if (!sink.complete() || sink.payload() != marker_payload)
      throw std::runtime_error("hostile shared marker replay bytes differ");
  ...
  {
    std::cerr << "shared-ownership-hostile-image-fixture: " << error.what()
              << '\n';
    return 2;
  }
}
