...
core_modules='libcrypto libpkgsource libpkgcatalog libpkgcatalog-acquire libpkgstate libpkgstate-posix libpkgstate-plan libpkgstate-apply libpkgfetch libpkgsource-exec libpkgbuild libpkgbuild-exec libpkgbuild-image libpkgsource-plan libpkgbuild-plan libpkgimage libpkgimage-exec libpkgobject libpkgplan libpkgexec libpkgapply libpkgapply-posix libpkgapply-exec libpkgresolve libpkgtransaction libpkgcheck libpkgcheck-exec'
...
pkg-config --exists \
  ...
  'libpkgobject >= 0.1.0' 'libpkgobject < 1.0.0' \
  ...
  'libpkgapply-posix >= 4.0.0' 'libpkgapply-posix < 5.0.0' \
  'libpkgapply-exec >= 3.0.2' 'libpkgapply-exec < 4.0.0' \
  ...
  'libpkgreconcile-apply >= 0.1.2' 'libpkgreconcile-apply < 1.0.0' \
  'libpkgreconcile-posix >= 0.1.0' 'libpkgreconcile-posix < 1.0.0' \
  'libpkgreconcile-apply-posix >= 0.1.2' 'libpkgreconcile-apply-posix < 1.0.0' \
...
"$srcdir/tests/integration/cli_run_root_authority_matrix_test.sh" "$tmp/pkgctl" \
  "$tmp/state-fixture" "$tmp/state-inspect-fixture" \
  "$tmp/run-evidence-inspect-fixture" "$tmp/native-runtime-root-fixture" \
  "$srcdir/tests/fixtures/collections/root-authority-matrix" \
  "$srcdir/tests/fixtures/native_root_view_fixture.sh"
...
"$srcdir/tests/integration/cli_build_runtime_cohort_test.sh" "$tmp/pkgctl" \
  "$tmp/state-fixture" "$tmp/state-inspect-fixture" \
  "$tmp/run-evidence-inspect-fixture" "$tmp/native-runtime-root-fixture" \
  "$srcdir/tests/fixtures/collections/runtime-cohort" \
  "$srcdir/tests/fixtures/native_root_view_fixture.sh"
